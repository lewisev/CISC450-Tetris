...
int main()
{
	...
	while (GameOn)
	{
		...
		if (n < 0)
		{
			...
			{
				// Don't start game and move blocks until client has sent first command
				if (keyPressed != TRUE)
				{
					continue;
				}

				// No data received... continue execution
		...
		*/
		...
		// User sent the first key, start playing game
		if (send_frame.sequence >= 0)
		{
			keyPressed = TRUE;
		}
